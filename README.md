...
*Not: `localhost:5001` Catalog Service'in çalıştığı varsayılan porttur. `docker-compose.yml` dosyasından kontrol edilebilir.*
 ...
 *   Zaman bazı gereksinimler tamamlanamadı. Kısa süre içerisinde geliştirmelere devam edilecek.
   * Antiforgery ile yaşanan bir sorun çözülecek ve program kullanılabilir hale gelecek.
   * Ön yüzde düzeltmeler yapılacak. Login olunmaması durumunda ilk olarak login sayfasına yönlendirme yapılacak. Login olunduktan sonra istenilen sayfaya yönlendirme yapılacak.
   * Testler tamamlanacak.
   ...
   * Loglama ve health check endpointleri eklenecek. Serilog ile loglama yapılacak.
