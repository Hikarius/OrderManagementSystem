...
{
    public class DecreaseStockCommand : ICommand<Result<bool>>
    {
        public List<DecreaseItemStock> Items { get; set; } = new List<DecreaseItemStock>();
    }
    ...
    public class DecreaseStockHandler(ProductRepository repository, IDistributedCache cache) : ICommandHandler<DecreaseStockCommand, Result<bool>>
    {

        private readonly ProductRepository _repository = repository;
        private readonly IDistributedCache _cache = cache;

        public async Task<Result<bool>> Handle(DecreaseStockCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                foreach (var item in request.Items)
                {
                    ...
                    {
                        await _repository.UnitOfWork.RollbackAsync(cancellationToken);
                        return new Result<bool> { IsSuccess = false, ErrorMessage = $"Insufficient stock for product {item.ProductId}", Value = false };
                    }
                }
